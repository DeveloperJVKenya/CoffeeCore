...
class EudrComplianceData {
  ...
  factory EudrComplianceData.fromMap(Map<String, dynamic> map) {
    return EudrComplianceData(
      ...
      treeCoverPercent2000:
          (map['treeCoverPercent2000'] as num? ?? 0).toDouble(),
      treeCoverLossAreaHa: (map['treeCoverLossAreaHa'] as num? ?? 0).toDouble(),
      remainingTreeCoverPercent:
          (map['remainingTreeCoverPercent'] as num? ?? 0).toDouble(),
...
class FarmPolygon {
  ...
  factory FarmPolygon.fromFirestore(
    ...
    return FarmPolygon(
      ...
      satelliteData: data['satelliteData'] != null
          ? SatelliteData.fromMap(data['satelliteData'] as Map<String, dynamic>)
          : null,
      activeCycleId: data['activeCycleId'] as String?,
    );
  }
  ...
  Map<String, dynamic> toFirestore() {
    return {
      ...
      if (activeCycleId != null) 'activeCycleId': activeCycleId,
    };
  }
  ...
  FarmPolygon copyWith({
    ...
    return FarmPolygon(
      ...
      agroMonitoringPolyId: agroMonitoringPolyId ?? this.agroMonitoringPolyId,
  ...
  LatLng get center {
    ...
    final lat = coordinates.map((c) => c.latitude).reduce((a, b) => a + b) /
        coordinates.length;
    final lng = coordinates.map((c) => c.longitude).reduce((a, b) => a + b) /
        coordinates.length;
...
class ClimateData {
  ...
  factory ClimateData.fromMap(Map<String, dynamic> map) {
    return ClimateData(
      temperatureCelsius: (map['temperatureCelsius'] as num? ?? 0).toDouble(),
...
class SatelliteData {
  ...
  factory SatelliteData.fromMap(Map<String, dynamic> map) {
    return SatelliteData(
      ...
      vegetationHealth: map['vegetationHealth'] as String? ?? 'Unknown',
      soilMoistureIndex: (map['soilMoistureIndex'] as num? ?? 0).toDouble(),
      fetchedAt: map['fetchedAt'] != null
          ? (map['fetchedAt'] as Timestamp).toDate()
          : DateTime.now(),
